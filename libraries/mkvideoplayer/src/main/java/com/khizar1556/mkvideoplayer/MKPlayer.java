...
public class MKPlayer {
    ...
    private final View.OnClickListener onClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            } else if (v.getId() == R.id.app_video_lock) {
                lock = true;

                showLockControl(true);
            } else if (v.getId() == R.id.app_video_unlock) {
                lock = false;

                showLockControl(true);
            } else if (v.getId() == R.id.app_video_box) {
                //hideNavigationBar();
...
//                Toast.makeText(activity,"previous",Toast.LENGTH_LONG).show();
    ...
    private void updatePausePlay() {
        if (videoView.isPlaying()) {
            $.id(R.id.app_video_play).image(R.drawable.hplib_ic_pause);
        } else {
            $.id(R.id.app_video_play).image(R.drawable.hplib_ic_play_download);
        }
    }
    ...
    public MKPlayer(final Activity activity) {
        try {
            IjkMediaPlayer.loadLibrariesOnce(null);
            IjkMediaPlayer.native_profileBegin("libijkplayer.so");
            playerSupport = true;
        } catch (Throwable e) {
            Log.e("MKPlayer", "loadLibraries error", e);
        }
...
//                        Toaster.show("download rate:" + extra);
        ...
        $.id(R.id.app_video_lock).clicked(onClickListener);
        ...
        $.id(R.id.app_video_unlock).clicked(onClickListener);
        $.id(R.id.app_video_box).clicked(onClickListener);
...
/*
    public void playList(ArrayList<VideoModel>,String url) {
        this.url = url;
        if (playerSupport) {
            $.id(R.id.app_video_loading).visible();
            videoView.setVideoPath(url);
...
*/
    ...
    /**
     * using constants in MKPlayer,eg: MKPlayer.SCALETYPE_FITPARENT
     *
     * @param scaleType
     */
     ...
     */
    public MKPlayer seekTo(int msec, boolean showControlPanle) {
    ...
    public MKPlayer forward(float percent) {
    ...
    public MKPlayer onComplete(Runnable complete) {
    ...
    public MKPlayer onControlPanelVisibilityChang(OnControlPanelVisibilityChangeListener listener) {
     ...
     * @return
     */
    public MKPlayer live(boolean isLive) {
        this.isLive = isLive;
        return this;
    }
    ...
    public MKPlayer onControlPanelVisibilityChange(OnControlPanelVisibilityChangeListener listener) {
        this.onControlPanelVisibilityChangeListener = listener;
        return this;
    }
